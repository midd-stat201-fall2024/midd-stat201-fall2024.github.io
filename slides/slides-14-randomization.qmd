...
-  Write down "promote" on `r sum(decision == "promote")` cards and "not promote" on `r sum(decision == "not promote")` cards
...
for(b in 1:B){
  shuffled <- sample(cards)
  males <- shuffled[1:n_male]
  females <- shuffled[-c(1:n_male)]
  props_diff_vec[b] <- mean(females == "promote") - mean(males == "promote")
}
```

Repeat the previous `r B` times:
...
null_dist |>
  ...
  geom_dotplot(dotsize= 0.3, fill = "grey") +
...
::: {.column width="40%"}
-   Out of `r B` simulations under $H_{0}$, `r sum(null_dist$extreme)` resulted in difference in promotion rates as or more extreme than our observed
...
-   Because the observed p-value of `r p_val` is less than our significant level 0.5, we reject $H_{0}$. The data provide strong evidence of sex discrimination against female candidates by the male supervisors.
...
::: {.column width="40%"}
-   $\hat{p}_{C} = \frac{`r sum(cpr$group == "control" & cpr$outcome == "survived")`}{`r sum(cpr$group == "control")`} =$ `r p_hat_c`

-   $\hat{p}_{T} = \frac{`r sum(cpr$group == "treatment" & cpr$outcome == "survived")`}{`r sum(cpr$group == "treatment")`} =$ `r p_hat_t`
...
for(b in 1:B){
  ...
  # deal out n_c cards to control group
  control_sim <- shuffled[1:n_c]
  
  # deal out remaining n_t cards to treatment group
  treat_sim <- shuffled[-c(1:n_c)]
...
diff_prop_null |>
  ggplot(aes(x = diffs))+
  geom_dotplot(dotsize= 0.35, fill = "grey") +
  scale_y_continuous(NULL, breaks = NULL) +
  labs(x = "Null distribution of difference in survival rates (treat - control)") +
  theme(text = element_text(size= 24))
```
...
diff_prop_null |>
  ...
  guides(fill = "none") +
  scale_fill_manual(values = c("grey", "pink"))+
  labs(x = "Difference in proportions (null distribution)") +
  theme(text = element_text(size= 24))
