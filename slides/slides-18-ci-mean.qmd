...
1.  Point estimate: $\bar{x} = `r xbar`$
2.  Standard error: $\widehat{SE} = \frac{s}{\sqrt{n}}= \frac{`r s_age`}{\sqrt{`r n`}} = `r se_hat`$
3.  Critical value:
    -   $df = n-1 = `r df`$
...
1.  **Independence** (extended): need data within *and* between the two groups. E.g. the two data sets come from independent random samples or from a randomized experiment.
...
1.   point estimate: $\bar{x}_{1} - \bar{x}_{2}$
...
:::: {.column width="50%"}
::: fragment
If $\sigma_{1}$ and $\sigma_{2}$ [known]{.underline}:
:::
...
:::: {.column width="50%"}
::: fragment
If $\sigma_{1}$ and $\sigma_{2}$ [unknown]{.underline}:
:::

2.  $\widehat{\text{SE}} = \sqrt{\frac{s_{1}^2}{n_{1}} + \frac{s_{2}^2}{n_{2}}}$
