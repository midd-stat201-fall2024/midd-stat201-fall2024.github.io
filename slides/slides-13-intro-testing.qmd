...
Research question: do the majority of STAT 201A/STAT 201B students get more than 7 hours of sleep?
...
-   Imagine a big bowl of marbles with orange and blue marbles.

    -   Orange marbles = people who got at least 7 hours

    -   Blue marbles = people who got less than 7 hours

-   What proportion of the marbles in the bowl should be orange vs blue?

    -   **To simulate under** $H_{0}$, no more than 50% of the marbles should be orange.

        -   We want convincing evidence even in the most "borderline" case, so we will choose 50% of the marbles to be orange.

-   This resulting bowl of marbles is our **null distribution**: what the population would look like if $H_{0}$ were true

## Simulating under $H_{0}$ (step 3 cont.)

-   We now replicate our original sample, this time sampling from this null distribution

    -   We repeatedly take samples from the null distribution, using original sample size $n =$ `r n`

    -   For each sample, calculate the simulated proportion of orange marbles, $\hat{p}_{sim}$
...
p_null_vec <- rep(NA, B)
for(b in 1:B){
  null_samp <- sample(c("orange", "blue"), size = n, replace = T, prob = c(0.5, 0.5))
  p_null_vec[b] <- sum(null_samp == "orange")/n
}
```
:::

-   Let's talk about what we're doing in lines 6 and 7!

## Null distribution of statistic

Repeating this many times, we can visualize the distribution of $\hat{p}_{sim}$ assuming $H_{0}$ true:
...
-   The resulting distribution is the called the **null distribution** for the particular sample statistic
...
We can directly obtain the p-value using our null distribution and observed $\hat{p}$!

-   We find the proportion of simulated sample proportions that were "as or more extreme" as our observed proportion of $\hat{p} =$ `r p_a` or $\hat{p} =$ `r p_b`
...
-   Out of `r B` replications, we saw `r sum(p_null_vec >= p_a)` instances $\hat{p}_{sim} \geq \hat{p}$
...
2.  Make a decision about research claim/question by comparing p-value to significance level $\alpha$
    ...
    -   ::: discuss
        Since our p value is less than $\alpha = 0.05$, we fail to reject $H_{0}$. The data do not provide sufficient evidence to suggest that the majority of STAT 201A/STAT 201B students get less than 7 hours of sleep.
        :::
...
3.  Obtain p-value by modeling randomness that would occur if the $H_{0}$ were true
