...
The observed/sample mean housing price is \$`r xbar` from a sample of `r n` houses.
...
-   In order to center this distribution at $\mu_{0} = 50$, just subtract $`r xbar` - 50 = `r xbar - 50`$ to every single bootstrapped mean
...
::: {.column width="40%"}
-   `r sum(null_dist_df$extreme)` out of `r B` simulated sample means are as or more extreme than the observed $\bar{x}$

-   p-value $\approx `r p_val`$
:::
:::::
::::::
...
-   Since our p-value of `r p_val` is less than the significance level of 0.05, we reject $H_{0}$. We have convincing evidence to suggest that the true average housing price of homes in Duke Forest in 2020 was not \$50k.
